...
public:
    ...
    void run()
    {
        ...
        shaderManager->addShader("samples/06_tlas_rebuilding/raygen.rgen.spv", vkss::eRaygenKHR, "main", vksgt::eGeneral);
        shaderManager->addShader("samples/06_tlas_rebuilding/miss.rmiss.spv", vkss::eMissKHR, "main", vksgt::eGeneral);
        shaderManager->addShader("samples/06_tlas_rebuilding/closesthit.rchit.spv", vkss::eClosestHitKHR, "main", vksgt::eTrianglesHitGroup);
        ...
        while (!window->shouldClose()) {
            ...
            double deltaTime = std::chrono::duration_cast<std::chrono::milliseconds>(curTime - lastTime).count();
            lastTime = curTime;
            ...
            worldMatrix = glm::rotate(glm::radians(float(deltaTime)), glm::vec3(0, 1, 0)) * worldMatrix;
            instance.transformMatrix = worldMatrix;
            tlas->update(*device, instance);
